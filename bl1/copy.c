...
typedef int(*func_nand_read_page)(u32, u32, u8*);
void copy_nand(u32 addr, u8 * buf, int len)
{
	int pageNo;
	
	pageNo = (addr/2048);
	
	func_nand_read_page nand_read_page = (func_nand_read_page)(*(u32 *)(0xD0037F90));
	
	while(len >= 2048)
	{
		nand_read_page(pageNo/64, pageNo%64, buf);
		pageNo++;
		len = len - 2048;
		buf = buf + 2048;
	}
